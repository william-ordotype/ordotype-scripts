...
| `parrainage-gate.js` | Referral offer page (slug `3-mois-50-parrainage`): loaded instead of `countdown.js` and `shared/stripe-checkout.js`. Without an `?invitation=` code it shows an "invited colleagues only" screen; with one it keeps the code through signup, hides the native Memberstack button and sends `offer: parrainage-3m` + `promotionCode` to `create-checkout-session`, which validates the code server-side. The page carries no coupon. |
...
- A form submitted while the field is still unbuilt (hidden the whole time) is reported once per page and never blocked: the raw value posts, but it does not go unnoticed.
