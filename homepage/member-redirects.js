...
(function() {
    ...
    (function() {
        ...
        // (modules, extra storage) are deliberately absent: losing an add-on must not
        // lock the whole site, so a hard redirect there would be wrong.
        //
        // Listed explicitly rather than matched on a prefix. Naming does not separate
        // the two families reliably — some account plans do not start with
        // `pln_compte-`, and at least one add-on does — and the two mistakes are not
        // symmetrical: a missing entry only means the member is not warned, while a
        // wrong match locks a paying member out of the product.
        //
